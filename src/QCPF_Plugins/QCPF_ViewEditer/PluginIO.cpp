...
{
    instance = this;
    PluginIO::I_PluginID = tr("QCFViewEditer");
    PluginIO::I_PluginType = PT_SYS;
    PluginIO::I_PluginAliasName = tr("View Editer");
...
void PluginIO::InitWidgetList()
{
    ...
    nFormInfo->_widget = new ViewEditer(_view);
    nFormInfo->_widget->setObjectName(QStringLiteral("wdt_ViewEditer"));
 ...
 *  action 函数指针所对应的回调函数
 * *********************************************************************/
void ShowViewEditer(bool checkState)
{
    if(instance->I_WidgetList.count()>0)
    {
        foreach (PluginWidgetInfo* pwi, instance->I_WidgetList) {
            if(pwi->_widget->objectName() == QStringLiteral("wdt_ViewEditer"))
            {
