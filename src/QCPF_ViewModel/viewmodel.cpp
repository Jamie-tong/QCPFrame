...
#include <QDataStream>
#include <QDir>
#include "../QCFModel/coreModel.h"
#include <QtWidgets/QMainWindow>
#include <QDockWidget>

QCFModel* _instCore;
QCFViewModel::QCFViewModel(QObject* parent):_config(this)
{
    this->setParent(parent);
    slot_LoadConfigFile(_config);
}

QCFViewModel::QCFViewModel(QObject* parent, QString configDirPath, QString configFileName):_config(this)
{
    this->setParent(parent);
...
int QCFViewModel::compareFiles(QString filePath1, QString filePath2)
{
...
int QCFViewModel::saveConfigFile(QString filePath)
{
...
int QCFViewModel::slot_LoadConfigFile(viewConfigModel &config)
{
    config.resetData();
...
int QCFViewModel::slot_SaveConfigFile()
{
    _outputInfo._type = InfoType::INFT_STATUS_INFO;
...
int QCFViewModel::slot_ApplyConfig()
{
    _outputInfo._type = InfoType::INFT_STATUS_INFO;
...
int QCFViewModel::slot_CancelConfig()
{
    _outputInfo._type = InfoType::INFT_STATUS_INFO;
...
int QCFViewModel::slot_Initialize(QString user, QString pwd, QString extInfo)
{
    _instCore = (QCFModel*)_core;
    if(user!="tt" || pwd!="1")
    {
...
void QCFViewModel::parseMenu(QMenu* nMenu, JMenuNode* nNode)
{
    if(nNode->_children.count()==0)
    {
        ...
        else
        {
            ...
            if(_instCore->I_UserInfo.CurrUserAuthority <= nNode->_menuAuthority)
...
void QCFViewModel::InitUIFromConfig(QMainWindow* viewHost)
{
...
void QCFViewModel::drawMenuFromConfig(QMenuBar* mainMenu)
{
    for(int i=0; i<_config._menuTopItemLst.count(); i++)
    {
        ...
        /*
            用户管理还没有实现，这里先暂时设置为开发者权限
        */
        _instCore->I_UserInfo.CurrUserAuthority = AT_DEVELOPER;
        if(_instCore->I_UserInfo.CurrUserAuthority <= _config._menuTopItemLst[i]->_menuAuthority)
...
void QCFViewModel::drawToolBarFromConfig(QList<QToolBar*> mainToolbarLst)
{
    for(int i=0; i<_config._toolBarLst.count(); i++)
    {
        ...
        for(int j=0; j<_config._toolBarLst[i]->_toolBarItemList.count(); j++)
        {
            ...
            else if(_config._toolBarLst[i]->_toolBarItemList[j]->_type == TP_WIDGET)//"Widget"
            {
                ...
                QVector<PluginInterface*> tPluginVec;
                if(pluginType == "System")
                    tPluginVec = ((QCFModel*)_core)->I_SysPlugins_Sel;
                else
                    tPluginVec = ((QCFModel*)_core)->I_NSysAllValidPlugins;

                foreach (PluginInterface* pi, tPluginVec) {
...
void QCFViewModel::drawStatusBarFromConfig(QStatusBar* mainStatusbar)
{
...
void QCFViewModel::removeWidgetsFromStatusBar(QStatusBar* mainStatusbar)
{
...
void QCFViewModel::drawDockWidgetFromConfig(QMainWindow* viewHost)
{
    foreach (WidgetItem* wi, _config._workspaceWidgetLst) {
        ...
        if(wi->_pluginType == PT_SYS)
            tPluginVec = ((QCFInterface*)_core)->I_SysPlugins;
        else
            tPluginVec = ((QCFInterface*)_core)->I_NSysAllValidPlugins;
